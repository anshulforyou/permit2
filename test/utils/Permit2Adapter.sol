...
import {Token} from "../../src/module/permit2-adapter/libraries/Token.sol";
...
library Utils {
    ...
    function buildPermit(address _token, uint256 _amount, uint256 _nonce, bytes memory _signature)
        internal
        pure
        returns (IArbitraryExecutionPermit2Adapter.SinglePermit memory _permit)
    {
    ...
    ) internal pure returns (IArbitraryExecutionPermit2Adapter.SinglePermit memory _permit) {
    ...
    function buildBatchPermit(address _token, uint256 _amount, uint256 _nonce, bytes memory _signature)
        internal
        pure
    ...
    function buildAllowanceTargets(address _target, address _token)
        internal
        pure
        returns (IArbitraryExecutionPermit2Adapter.AllowanceTarget[] memory _allowanceTargets)
    {
    ...
    function buildContractCalls(address _target, bytes memory _data, uint256 _value)
        internal
        pure
        returns (IArbitraryExecutionPermit2Adapter.ContractCall[] memory _contractCalls)
    {
        ...
        _contractCalls[0] =
            IArbitraryExecutionPermit2Adapter.ContractCall({target: _target, data: _data, value: _value});
    }
    ...
    function buildTransferOut(address _token, Token.DistributionTarget[] memory _distribution)
        internal
        pure
        returns (IArbitraryExecutionPermit2Adapter.TransferOut[] memory _transferOut)
    {
    ...
    function buildDistribution(address _recipient1, uint256 _shareBps1, address _recipient2)
        internal
        pure
        returns (Token.DistributionTarget[] memory _distribution)
    {
